  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      <div className="max-w-4xl mx-auto bg-white min-h-screen pb-24">
                              ...
                              <div className="px-5">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                  {group.parameters.map(param => (
                                    <div key={param.id} className={`bg-white rounded-2xl p-4 border-2 border-slate-200 hover:${colors.border} shadow-sm cursor-pointer transition-all`}>
        ...
        {/* Floating Action Bar (Bottom Nav) - Premium App style */}
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-48px)] max-w-md z-50">
          <div className="bg-white/90 backdrop-blur-xl border border-slate-200 rounded-[24px] p-2 flex justify-between shadow-[0_20px_40px_rgba(0,0,0,0.1)]">
            <button onClick={() => setActiveTab('reports')} className={`flex-1 flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${activeTab === 'reports' ? 'bg-blue-50 text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
              <FileText size={20} strokeWidth={activeTab === 'reports' ? 2.5 : 2} />
              <span className="text-[10px] font-bold tracking-wide">Insights</span>
            </button>
            <button onClick={() => setActiveTab('compare')} className={`flex-1 flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${activeTab === 'compare' ? 'bg-blue-50 text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
              <Activity size={20} strokeWidth={activeTab === 'compare' ? 2.5 : 2} />
              <span className="text-[10px] font-bold tracking-wide">Compare</span>
            </button>
            <button onClick={() => setActiveTab('rx')} className={`flex-1 flex flex-col items-center gap-1 p-2 rounded-2xl transition-all ${activeTab === 'rx' ? 'bg-blue-50 text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
              <Pill size={20} strokeWidth={activeTab === 'rx' ? 2.5 : 2} />
              <span className="text-[10px] font-bold tracking-wide">Therapy</span>
            </button>
            <button className="flex-1 flex flex-col items-center gap-1 p-2 rounded-2xl text-slate-400 hover:text-slate-600 transition-all">
              <User size={20} />
